...
let root_search board white_to_move last_move castling_right board_record half_moves depth ply evaluation zobrist_position move_list =
  incr node_counter;
  ...
  if not !stop_calculation then begin
    ...
    in store transposition_table zobrist_position Pv depth stored_value !best_move !go_counter
  end;
  !best_score
