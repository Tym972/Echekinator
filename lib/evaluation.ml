...
let gamephase_table = [|0; 0; 1; 1; 2; 4; 0|]

let mob_center = [| 0; 0; -4; -6; -6; -13; 0|]
let mob_step_mg = [| 0; 0; 0; 5; 3;  2; 0|]
let mob_step_eg = [| 0; 0; 1; 5; 3;  5; 0|]
...
let hce position =
  ...
  in for side = 0 to 1 do
    ...
    while !bitboard <> 0L do
      ...
      let index =  (from lxor (56 * side))  + 64 * (pawn - 1) in
      ...
      in if (ahead.(from + 64 * side) &&& enemy_pawns = 0L) then begin
        ...
        bonus_eg := !bonus_eg + 12 * (distance_tab.(from).(lsb_index pieces_bitboard.(king + 6 * (side lxor 1))) - distance_tab.(from).(king_square));
    ...
    while !bitboard <> 0L do
      let from, other_pieces_bitboard = pop_lsb !bitboard in
      bitboard := other_pieces_bitboard;
      let index =  (from lxor (56 * side))  + 64 * (knight - 1) in
      add_score mg_pst.(index) eg_pst.(index) sign;
    ...
    while !bitboard <> 0L do
      let from, other_pieces_bitboard = pop_lsb !bitboard in
      bitboard := other_pieces_bitboard;
      let index =  (from lxor (56 * side)) + 64 * (bishop - 1) in
      add_score mg_pst.(index) eg_pst.(index) sign;
    ...
    while !bitboard <> 0L do
      let from, other_pieces_bitboard = pop_lsb !bitboard in
      bitboard := other_pieces_bitboard;
      let index =  (from lxor (56 * side))  + 64 * (rook - 1) in
      add_score mg_pst.(index) eg_pst.(index) sign;
    ...
    while !bitboard <> 0L do
      let from, other_pieces_bitboard = pop_lsb !bitboard in
      bitboard := other_pieces_bitboard;
      let index =  (from lxor (56 * side))  + 64 * (queen - 1) in
      add_score mg_pst.(index) eg_pst.(index) sign;
    ...
    let index =  (if side = 0 then king_square else king_square lxor 56)  + 64 * (king - 1) in
    add_score mg_pst.(index) eg_pst.(index) sign;
