...
let hided_squares = [| 0xE7L; 0xE700000000000000L|]
...
let hce position =
  ...
  for side = 0 to 1 do
    ...
    (*if pieces_bitboard.(king + 6 * side) &&& hided_squares.(side) <> 0L then begin
      let shield = ref 0 in
      ...
      for file = min_file to max_file do
        ...
        if file_pawns <> 0L then begin
          let rank =
            ...
            else begin print_endline (string_of_int (lsb_index file_pawns) ^" " ^ string_of_int (msb_index file_pawns)); print_bitboard file_pawns;
      ...
      print_endline (string_of_int !shield);
      mg_score := !mg_score + (-2 * side + 1) * !shield;
    end*)
    (*let rooks_bitboard = ref pieces_bitboard.(rook + 6 * side) in
    while !rooks_bitboard <> 0L do
      let from, other_rook = pop_lsb !rook_bitboard in
      rooks_bitboards := other_rook;
      let file = files.(from land 7) in
      if pieces_bitboard.(pawn + 6 * side) &&& file = 0L then begin
        add_score 44 (-3) (-2 * side + 1)
        if pieces_bitboard.(pawn + 6 * (side lxor 1)) &&& file = 0L then begin
